...
exports.book_detail = asyncHandler(async (req, res, next) => {
    const [book] = await Promise.all([Book.findById(req.params.id).populate("author").exec()]);
...
exports.book_create_get = asyncHandler(async (req, res, next) => {
    ...
    const [allAuthors] = await Promise.all([Author.find().sort({ family_name: 1}).exec()]);
...
exports.book_delete_get = asyncHandler(async (req, res, next) => {
    const [book] = await Promise.all([Book.findById(req.params.id).exec()]);
